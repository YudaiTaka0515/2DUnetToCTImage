...
def ConvertDicom2Png(dicom_folder, save_parent_dir):
    ...
    for dicom_dir in dicom_dir_list:
        ...
        SaveDicomAsPng(dicom_dir, save_dir)

    print("Done")


# 先頭から15枚のみ読み込む
def SaveDicomAsPng(dicom_dir, save_dir, n_to_be_loaded=15):
    """
...
def LoadImageFromDicom(dicom_paths, n_to_be_loaded=15):
...
# 先頭から15枚のみ読み込む
def SaveMaskAsPng(mask_dir, save_parent_dir, n_to_be_loaded=15):
...
def main_dicom():
    dicom_folder_ = r"I:\Data\OPE(25ー59）"
    save_parent_dir_ = r"I:\Data\ForTrachea\IMAGES"
...
def main_raw():
    data_folder_ = r"I:\Data\ForTrachea\RAW"
    save_parent_dir_ = r"I:\Data\ForTrachea\MASKS"
    if not os.path.exists(save_parent_dir_):
        os.mkdir(save_parent_dir_)
